...
static esp_err_t parse_mqtt_config(const cJSON *config_json, mqtt_manager_config_t *mqtt_config)
{
    ...
    if (cJSON_IsString(user))
        strncpy(mqtt_config->username, user->valuestring, sizeof(mqtt_config->username) - 1);

    const cJSON *pass = cJSON_GetObjectItem(config_json, "password");
    if (cJSON_IsString(pass))
        strncpy(mqtt_config->password, pass->valuestring, sizeof(mqtt_config->password) - 1);

    const cJSON *prefix = cJSON_GetObjectItem(config_json, "base_topic_prefix");
    if (cJSON_IsString(prefix))
        strncpy(mqtt_config->base_topic_prefix, prefix->valuestring, sizeof(mqtt_config->base_topic_prefix) - 1);

    const cJSON *lwt_topic_json = cJSON_GetObjectItem(config_json, "lwt_topic");
    if (cJSON_IsString(lwt_topic_json))
        strncpy(mqtt_config->lwt_topic, lwt_topic_json->valuestring, sizeof(mqtt_config->lwt_topic) - 1);

    const cJSON *lwt_msg_json = cJSON_GetObjectItem(config_json, "lwt_message");
    if (cJSON_IsString(lwt_msg_json))
        strncpy(mqtt_config->lwt_message, lwt_msg_json->valuestring, sizeof(mqtt_config->lwt_message) - 1);

    const cJSON *lwt_qos_json = cJSON_GetObjectItem(config_json, "lwt_qos");
    if (cJSON_IsNumber(lwt_qos_json))
        mqtt_config->lwt_qos = lwt_qos_json->valueint;

    const cJSON *lwt_retain_json = cJSON_GetObjectItem(config_json, "lwt_retain");
    if (cJSON_IsBool(lwt_retain_json))
        mqtt_config->lwt_retain = cJSON_IsTrue(lwt_retain_json);
