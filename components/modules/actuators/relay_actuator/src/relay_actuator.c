...
#include "service_locator.h" // For fmw_service_get and fmw_service_lookup_by_type
...
static esp_err_t parse_relay_config(const cJSON *config, relay_private_data_t *private_data) {
    ...
    if (!cJSON_IsNumber(pin_node)) return ESP_ERR_INVALID_ARG;
    ...
    if (cJSON_IsString(state_node)) {
        if (strcmp(state_node->valuestring, "on") == 0) private_data->initial_state = INITIAL_STATE_ON;
        else if (strcmp(state_node->valuestring, "last") == 0) private_data->initial_state = INITIAL_STATE_LAST;
        else private_data->initial_state = INITIAL_STATE_OFF;
    } else {
