...
- **feat(sensors):** 🚀 დაემატა ახალი, სრულფასოვანი მოდული **`time_of_flight_sensor`** VL6180X ჩიპისთვის.
  - **I2C ინტეგრაცია:** მოდული უკავშირდება VL6180X სენსორს `i2c_bus` სერვისის მეშვეობით, კითხულობს მის ID-ს და ასრულებს სრულ ინიციალიზაციას datasheet-ის მიხედვით.
  - **პერიოდული გაზომვები:** `system_timer` სერვისის გამოყენებით, მოდული პერიოდულად (polling რეჟიმში) ზომავს მანძილს და შედეგს მილიმეტრებში აქვეყნებს `Event Bus`-ზე `SENSOR_DATA_DISTANCE_MM` ივენთით.
  - **ეკოსისტემასთან თავსებადობა:** გამოქვეყნებული ივენთი სრულად თავსებადია `sensor_aggregator` მოდულთან, რაც უზრუნველყოფს მონაცემების მარტივ აგრეგაციას და MQTT-ით გადაცემას.

### 🐛 გამოსწორებული შეცდომები (Fixed)

- **fix(i2c_bus):** 🛠️ გასწორდა `i2c_bus` მოდულის სკანირების ლოგიკა. დიაგნოსტიკური სკანირება ახლა კორექტულად სრულდება `init` ფაზაში, რაც ამარტივებს I2C მოწყობილობების აღმოჩენას და გამართვას.
...
- **refactor(ble_prov):** 🆔 `ble_provisioning` მოდული ახლა დინამიურად აგენერირებს BLE სარეკლამო სახელს `device_identity_service`-ის გამოყენებით.
  - საბოლოო სახელი იღებს `Prefix-DeviceID` ფორმატს (მაგ., `Synapse-5C013B4A9954`), რაც უზრუნველყოფს თითოეული მოწყობილობის უნიკალურობას ქსელში.
  - **არქიტექტურული შესწორება:** სერვისზე დამოკიდებული ლოგიკა გადატანილ იქნა `_create` ფუნქციიდან `_init` ფაზაში და დაკორექტირდა `init_level` პარამეტრები (`device_identity_service`: 20, `ble_provisioning`: 90), რათა გარანტირებული იყოს სერვისების ინიციალიზაციის სწორი თანმიმდევრობა.
  - გაუმჯობესდა მოდულის იზოლაცია `hardcoded` სერვისის სახელის (`main_identity_service`) `fmw_service_lookup_by_type`-ით ჩანაცვლებით.
- **docs(ble_prov):** ✍️ კოდი სრულად დოკუმენტირდა Doxygen სტანდარტის შესაბამისად, `production-ready` ხარისხის უზრუნველსაყოფად.

## [5.6.0] - 2025-08-16

### ✨ დამატებული ფუნქციონალი (Added)

- **feat(system):** 🌐 `time_sync` მოდული გარდაიქმნა "ყალბი" იმპლემენტაციიდან სრულფასოვან **SNTP კლიენტად**.
  - **ავტომატური სინქრონიზაცია:** WiFi-სთან დაკავშირების შემდეგ, მოდული ავტომატურად ასინქრონებს სისტემის დროს `pool.ntp.org` სერვერთან.
  - **დროის ზონის მხარდაჭერა:** მოდულს `config.json`-ში დაემატა `timezone` პარამეტრი. ის აყენებს სისტემურ `TZ` გარემოს ცვლადს, რაც უზრუნველყოფს UTC დროის კორექტულ გარდაქმნას ლოკალურ დროში.

### 🔄 შეცვლილი და გაუმჯობესებული ფუნქციონალი (Changed)

- **improve(ui):** 🕰️ `ui_manager` ახლა კორექტულად აჩვენებს დროს.
  - სანამ დრო არ დასინქრონდება, მთავარ ეკრანზე გამოჩნდება `"--:--"`.
  - სინქრონიზაციის შემდეგ, ეკრანზე აისახება ზუსტი, ლოკალური დრო, დროის ზონის გათვალისწინებით.
- **improve(ui):** 🎨 `ui_manager`-ის ციფრების ფონტს (`roboto_bold_24_digits.h`) დაემატა `-` (ტირე) სიმბოლოს მხარდაჭერა, რაც `"--:--"`-ის კორექტულად გამოტანას უზრუნველყოფს.
